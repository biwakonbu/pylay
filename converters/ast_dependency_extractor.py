...
class ASTDependencyExtractor:
        ...
        try:
            ...
            for base in node.bases:
                ...
                if base_name and base_name != class_name:
                    # 継承は強い依存
                    self._add_edge(class_name, base_name, 'inherits', weight=0.9)
    ...
    def _handle_function_def(self, node: ast.FunctionDef, file_path: str) -> None:
        ...
        for arg in node.args.args:
            if arg.annotation:
                ...
                if arg_type:
                    # 引数参照は中程度の依存
                    self._add_edge(func_name, arg_type, 'references', weight=0.6)
        ...
        if node.returns:
            ...
            if return_type:
                # 戻り値参照は強い依存
                self._add_edge(func_name, return_type, 'returns', weight=0.8)
    ...
    def _handle_method_def(self, node: ast.FunctionDef, class_name: str, file_path: str) -> None:
        ...
        for arg in node.args.args:
            if arg.annotation:
                arg_type = self._get_type_name_from_ast(arg.annotation)
                if arg_type:
                    self._add_edge(method_name, arg_type, 'references', weight=0.6)
        ...
        if node.value and isinstance(node.value, ast.Call):
            ...
            if hasattr(node.value.func, 'id'):
                ...
                if node.targets and hasattr(node.targets[0], 'id'):
                    ...
                    self._add_edge(var_name, func_name, 'calls')
    ...
    def _handle_annotation(self, node: ast.AnnAssign, class_name: str, file_path: str) -> None:
        ...
        if node.annotation:
            ...
            if annotated_type:
                self._add_edge(class_name, annotated_type, 'references')
