...
def yaml_to_spec(yaml_str: str, root_key: str | None = None) -> TypeSpec | TypeRoot:
    ...
    if isinstance(data, dict) and not root_key:
        if 'types' in data:
            ...
            # 参照解決を実行
            resolved_types = RefResolver.resolve_all(type_root.types)
            ...
            if len(data) == 1:
                ...
                RefResolver.detect_cycles_from_data(data)
...
def validate_with_spec(spec: TypeSpec | str, data: Any, max_depth: int = 10) -> bool:
    """
    TypeSpecに基づいてデータをバリデーション（非再帰版、深さ制限付き）。

    Args:
        spec: バリデーション対象のTypeSpecまたは参照文字列
        data: バリデーション対象のデータ
        max_depth: 再帰の最大深さ（デフォルト: 10）

    Returns:
        バリデーション成功の場合True
    """
    # 参照解決が必要な場合、簡易的にstrとして扱う（実際はTypeContextが必要）
    ...
    while stack:
        ...
        try:
            if isinstance(current_spec, DictTypeSpec):
                if not isinstance(current_data, dict):
                    return False
                for key, prop_spec in current_spec.properties.items():
                    if key in current_data:
                        stack.append((prop_spec, current_data[key], depth + 1))
            elif isinstance(current_spec, ListTypeSpec):
                ...
                for variant in current_spec.variants:
                    if isinstance(variant, TypeSpec):
                        if variant.type == 'str' and isinstance(current_data, str):
                            return True
                        elif variant.type == 'int' and isinstance(current_data, int):
                            return True
                        elif variant.type == 'float' and isinstance(current_data, (int, float)):
                            return True
                        elif variant.type == 'bool' and isinstance(current_data, bool):
                            return True
                        elif variant.type == 'any':
                            return True
                return False
            elif isinstance(current_spec, TypeSpec):
                # 基本型バリデーション
                if current_spec.type == 'str':
                    return isinstance(current_data, str)
                elif current_spec.type == 'int':
                    return isinstance(current_data, int)
                elif current_spec.type == 'float':
                    return isinstance(current_data, (int, float))
                elif current_spec.type == 'bool':
                    return isinstance(current_data, bool)
                elif current_spec.type == 'any':
                    return True
                else:
                    return False
        except Exception:
