...
def generate_dependency_docs(
    ...
    if analyze_graph:
        ...
        if dot_file:
            ...
            svg_file = dot_file.replace('.dot', '.svg') if dot_file.endswith('.dot') else f"{dot_file}.svg"
            svg_path = Path(svg_file)
            nx_adapter.export_visualization(dot_path, svg_path if svg_path != dot_path else None)
...
def main() -> None:
    ...
    parser.add_argument(
        "--graphml",
        help="GraphMLファイルの出力パス（--analyze使用時）"
    )
