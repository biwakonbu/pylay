...
class GraphDocGenerator(DocumentGenerator):
    ...
    def _generate_edge_table(self, edges: list[GraphEdge]) -> None:
        ...
        for edge in edges:
            ...
            self.md.paragraph(f"| {edge.source} | {edge.target} | {edge.relation_type} | {edge.weight} | {strength} {is_strong} |")
    ...
    def generate_with_visualization(self, output_path: Path, graph: TypeDependencyGraph,
                                   dot_file: Optional[Path] = None) -> None:
        """
